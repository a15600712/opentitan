...
ACCUMULATOR_START = 0x0      # Code uses x2 = 0
MATRIX_A_START    = 0x800
N_A_BYTE_ADDR     = 0x30800
N_A_WIDTH_ADDR    = 0x30804
VECTOR_B_START    = 0x30820

# Sizes (Based on GF16 = 4 bits/element and dimensions derived)
ACCUMULATOR_SIZE_BYTES = 2048  # 4096 elements * 0.5 bytes/element
MATRIX_A_SIZE_BYTES = 196608   # 96 cols * 2048 bytes/col (n_A_byte)
VECTOR_B_SIZE_BYTES = 48       # 96 elements * 0.5 bytes/element
...
DMEM_DUMP_FILE = None  # e.g., "dmem.bin" - binary format

# Specify full path for DMEM hex dump file (None to disable)
# Examples:
# - "dmem_hex_dump.txt"  (current directory)
# - "/path/to/output/dmem_dump.txt"  (absolute path)
# - os.path.join(os.path.dirname(__file__), "output", "dmem_dump.txt")  (relative to script)
DMEM_HEX_DUMP_FILE = "/home/anon/dmem_hex_dump.txt"
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='OTBN Data Injection and Register Dump Script')
    parser.add_argument('--elf', type=str, help='Path to ELF file (overrides ELF_FILE in script)')
    parser.add_argument('--dmem-dump', type=str, help='Path for DMEM hex dump output (overrides DMEM_HEX_DUMP_FILE in script)')
    args = parser.parse_args()

    # Override configuration with command-line arguments if provided
    global ELF_FILE, DMEM_HEX_DUMP_FILE
    if args.elf:
        ELF_FILE = args.elf
        print(f"Using ELF file from command line: {ELF_FILE}")
    if args.dmem_dump:
        DMEM_HEX_DUMP_FILE = args.dmem_dump
        print(f"Using DMEM dump path from command line: {DMEM_HEX_DUMP_FILE}")
    ...
    # Dump DMEM in binary format if requested
    if DMEM_DUMP_FILE:
        try:
            with open(DMEM_DUMP_FILE, 'wb') as f:
                f.write(sim.dump_data())
            print(f"DMEM contents (binary) written to {DMEM_DUMP_FILE}")
        except Exception as e:
            print(f"Error writing DMEM binary dump file: {e}", file=sys.stderr)
    
    # Dump DMEM in hex format if requested
    if DMEM_HEX_DUMP_FILE:
        # Ensure directory exists
        dmem_hex_dir = os.path.dirname(DMEM_HEX_DUMP_FILE)
        if dmem_hex_dir and not os.path.exists(dmem_hex_dir):
            try:
                os.makedirs(dmem_hex_dir)
                print(f"Created directory: {dmem_hex_dir}")
            except Exception as e:
                print(f"Error creating directory for DMEM hex dump: {e}", file=sys.stderr)
                return 1
                
        try:
            with open(DMEM_HEX_DUMP_FILE, 'w') as f:
                dump_dmem_hex(sim, 0x0, 0x80000, f)  # Dump full DMEM or adjust range as needed
            print(f"DMEM contents (hex format) written to {DMEM_HEX_DUMP_FILE}")
        except Exception as e:
            print(f"Error writing DMEM hex dump file: {e}", file=sys.stderr)
